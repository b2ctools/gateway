...
import { AccountDto, sortable, accountToDto } from '../../domain/account.interface';
...
export class SearchAccountController {
  ...
  @Get()
  async findAllAccounts(@Query() request: SearchAccountRequest): Promise<SearchOutput<AccountDto>> {
    const items = (await this.useCase.execute(request)).map((s) => accountToDto(s));
    return {
      count: items.length,
      data: items,
      sortable
