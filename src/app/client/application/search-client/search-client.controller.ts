...
import { ClientDto, sortable, clientToDto } from '../../domain/client.interface';
...
export class SearchClientController {
  ...
  @Get()
  async findAllClients(@Query() request: SearchRequest): Promise<SearchOutput<ClientDto>> {
    const items = (await this.useCase.execute(request)).map((s) => clientToDto(s));
    return {
      count: items.length,
      data: items,
      sortable
