...
export class PermissionService {
  ...
  async updatePermission(id: ID, request: UpdatePermissionRequest): Promise<Permission> {
    const { name, description } = request;
    const existingPermission = await this.findByIdOrFail(id);

    existingPermission.name = name ? name : existingPermission.name;
    existingPermission.description = description ? description : existingPermission.description;
