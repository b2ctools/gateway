...
export class TokenService {
  ...
  validateAccessToken(token: string): AccessPayload {
    ...
    if (!userId || !tenantId || !email || !role || !session){
      ...
      const message = 'Error validing access token. Problems with the payload structure.'
  ...
  getRecoveryPasswordToken(code: string): Token {
    if (!code) throw new BadRequestException('Error getting recovery password token. Code was not provided.')
